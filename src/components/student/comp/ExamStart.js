      ...
      <div className="md:flex md:space-x-6">
        <div className="md:w-3/4 mb-6 md:mb-0 p-4 border-r">
          <h1 className="text-xl font-semibold mb-2">
            Q{currentQuestionIndex + 1} of {questions.length}  | Group: {question.group_title}
          </h1>
          {/* Display group name (group_title) above each question */}
         
          <h2 className="text-gray-700 mb-4 font-semibold" dangerouslySetInnerHTML={{ __html: question.question }}></h2>
          <div className="space-y-3">
            {question.options.map((option) => (
              <label key={option.oid} className="block p-3 border rounded-lg cursor-pointer hover:bg-gray-100">
                <input
                  type="radio"
          ...
          <div className="flex justify-between mt-6 space-x-2">
            <button className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 w-1/3" onClick={handleClearAnswer}>
              Clear Answer
            </button>
            <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 w-1/3" onClick={handleMarkForReview}>
              Mark for Review
            </button>
            <button className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 w-1/3" onClick={handleNext}>
              Save & Next
            </button>
          </div>
        </div>
        ...
        <div className="md:w-1/4 p-4">
          <div className="mb-6">
            ...
            <div className="grid grid-cols-4 gap-2">
              {questions.map((question) => (
                <button
                  key={question.qid}
                  className={`w-10 h-10 rounded-lg ${getStatusColor(questionStatus[question.qid])} text-white`}
                  onClick={() => setCurrentQuestionIndex(questions.findIndex(q => q.qid === question.qid))}
                >
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center mb-2">
              ...
              <span>Answered & Marked for Review: {statusCounts['Answered & Marked for Review']}</span>
            </div>
          ...
          <button className="bg-red-600 text-white w-full py-2 rounded-lg hover:bg-red-700" onClick={handleSubmit}>
            Finish Test
          </button>
        </div>
      </div>
